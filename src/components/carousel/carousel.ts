...
import { NAVIGATION_DIRECTION, CAROUSEL_SIZE } from './defs';
import BXSwitch from './carousel-item';
import styles from './carousel.scss';

export { NAVIGATION_DIRECTION, CAROUSEL_SIZE };
 ...
 * Content switcher.
 * @element bx-carousel
 * @fires bx-carousel-beingselected
 *   The custom event fired before a content switcher item is selected upon a user gesture.
 *   Cancellation of this event stops changing the user-initiated selection.
 * @fires bx-carousel-selected - The custom event fired after a a content switcher item is selected upon a user gesture.
 */
@customElement(`${prefix}-carousel`)
class BXCarousel extends LitElement {
  /**
   * Handles `mouseover`/`mouseout` events on `<slot>`.
   * @param event The event.
   */
  private _handleHover({ target, type }: MouseEvent) {
    const { selectorItem } = this.constructor as typeof BXCarousel;
    ...
    forEach(this.querySelectorAll(selectorItem), (elem, i) => {
      // Specifies child `<bx-carousel-item>` to hide its divider instead of using CSS,
  ...
  protected _getNextItem(currentItem: BXSwitch, direction: number) {
    const items = this.querySelectorAll((this.constructor as typeof BXCarousel).selectorItemEnabled);
  ...
  protected _handleUserInitiatedSelectItem(item: BXSwitch) {
    if (!item.disabled && item.value !== this.value) {
      ...
      const constructor = this.constructor as typeof BXCarousel;
  ...
  protected _navigate(direction: number) {
    const { selectorItemSelected } = this.constructor as typeof BXCarousel;
  ...
  protected _selectionDidChange(itemToSelect: BXSwitch) {
    ...
    forEach(this.querySelectorAll((this.constructor as typeof BXCarousel).selectorItemSelected), item => {
  ...
  static get selectorItemEnabled() {
    return `${prefix}-carousel-item:not([disabled])`;
  }
  ...
  static get selectorItemSelected() {
    return `${prefix}-carousel-item[selected]`;
  }
  ...
  static get eventBeforeSelect() {
    return `${prefix}-carousel-beingselected`;
  }
  ...
  static get eventSelect() {
    return `${prefix}-carousel-selected`;
  }
