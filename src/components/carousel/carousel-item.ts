 ...
 */
@customElement(`${prefix}-carousel-item`)
class BXCarouselItem extends FocusMixin(LitElement) {
  /**
  ...
  /**
   * The `value` attribute that is set to the parent `<bx-carousel>` when this carousel item is selected.
   */
  @property()
  value = '';
  ...
  render() {
    ...
    const className = classMap({
      [`${prefix}--carousel-btn`]: true,
      [`${prefix}--carousel--selected`]: selected,
    });
    return html`
      <button
        type="button"
        ...
        <span class="${prefix}--carousel__label"><slot></slot></span>
      </button>
    `;
  }
